...
export const classifyText = async (text: string, existingTags: string[] = [], retryCount = 0): Promise<Partial<BrainDumpItem>[]> => {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      2. SHOPPING: for ALL Life Admin, Chores, Errands, and Purchases.
      ...
      - ALWAYS generate relevant 'tags'.
      - Check against existing tags list provided.
      
      For SHOPPING (Life Admin) items:
      - determine 'shoppingCategory': 'urgent', 'routine', or 'not_urgent'.
      - If 'routine', guess 'recurrenceDays' (default 7).
      - TIME-BOUND: If specific deadline exists, extract to 'meta.date'.
        ...
        responseSchema: {
          ...
          items: {
            ...
            properties: {
              ...
              content: {
                type: Type.STRING,
                description: "The cleaned up content text for this specific item",
              },
              meta: {
                type: Type.OBJECT,
                properties: {
