    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-surface border border-border rounded-xl w-full max-w-sm shadow-2xl p-6">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-lg font-bold text-white">{mode === 'add' ? 'Add Wallet' : 'Edit Wallet'}</h3>
                        ...
                        <div className="flex-1 flex gap-1 overflow-x-auto no-scrollbar">
                             {COLORS.map(c => (
                                 <button 
                                    key={c}
                                    onClick={() => setColor(c)}
                                    className={`w-4 h-4 rounded-full ${c} ${color === c ? 'ring-2 ring-white' : 'opacity-50 hover:opacity-100'}`}
                                 />
                             ))}
                        </div>
                    </div>
