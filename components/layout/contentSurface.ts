...
export const contentSurface = {
  ...
  headerHero: 'mx-0 mt-3 mb-5 rounded-[28px] bg-surface p-5 pt-6 text-primary shadow-sm ring-1 ring-inset ring-border/70 touch-pan-y sm:p-6 lg:mt-6 lg:rounded-[30px] lg:p-7',
  ...
  workspaceHeaderGrid: 'grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start',
  workspacePeriodControl: 'w-full shrink-0 rounded-2xl bg-background/55 px-2 py-2 ring-1 ring-inset ring-border/70 touch-pan-y sm:w-auto sm:min-w-60 sm:px-3 lg:px-4 lg:py-3',
  workspacePeriodButton: 'flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-muted transition-colors hover:bg-surface hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/60',
  workspacePeriodLabel: 'flex min-w-20 flex-col items-center overflow-hidden px-2 text-center sm:min-w-24',
  workspacePeriodKicker: 'mb-1 text-[10px] font-semibold uppercase leading-none tracking-[0.14em] text-muted',
  workspacePeriodTitle: 'text-sm font-semibold leading-none text-primary sm:text-base lg:text-lg',
  ...
  workflowPanel: 'rounded-[28px] bg-surface p-4 shadow-sm ring-1 ring-inset ring-border/70',
  desktopWorkflowPanel: 'lg:rounded-[28px] lg:bg-surface lg:p-4 lg:shadow-sm lg:ring-1 lg:ring-inset lg:ring-border/70',
  ...
  emptyStateCard: 'rounded-[28px] border border-dashed border-border/90 bg-surface p-6 text-center shadow-sm lg:p-8',
  ...
  calendarFrame: 'w-full overflow-hidden rounded-[28px] bg-surface shadow-sm ring-1 ring-inset ring-border/70',
  ...
  card: 'rounded-[24px] bg-surface shadow-sm ring-1 ring-inset ring-border/65',
} as const;
...
export const addItemModal = {
  panel: `${responsiveModal.denseFormPanel} max-h-[90dvh]`,
  header: 'flex shrink-0 items-center justify-between border-b border-border/60 bg-surface px-5 py-4 sm:px-6',
  title: 'flex items-center gap-2 text-lg font-semibold tracking-tight text-primary sm:text-xl',
  icon: 'w-5 h-5 text-indigo-500',
  closeButton: 'flex min-h-11 min-w-11 items-center justify-center rounded-xl text-muted transition-colors hover:bg-black/[0.04] hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/60 focus-visible:ring-offset-2 focus-visible:ring-offset-surface dark:hover:bg-white/[0.06]',
  body: 'space-y-5 overflow-y-auto p-5 sm:p-6',
  footer: 'shrink-0 border-t border-border/60 bg-surface/98 p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] backdrop-blur-xl sm:p-6',
  label: 'mb-1.5 block text-xs font-semibold text-primary',
  input: 'min-h-11 w-full rounded-xl border border-border/80 bg-background/70 px-3.5 py-2.5 text-primary font-medium outline-none transition focus:border-indigo-500/60 focus:ring-4 focus:ring-indigo-500/10',
  titleInput: 'min-h-12 w-full rounded-xl border border-border/80 bg-background/70 px-3.5 py-2.5 text-lg font-semibold tabular-nums tracking-tight text-primary outline-none transition focus:border-indigo-500/60 focus:ring-4 focus:ring-indigo-500/10',
  ...
  select: 'min-h-11 w-full appearance-none rounded-xl border border-border/80 bg-background/70 px-3.5 py-2.5 font-medium text-primary outline-none transition focus:border-indigo-500/60 focus:ring-4 focus:ring-indigo-500/10',
  smallInput: 'min-h-11 w-full rounded-xl border border-border/80 bg-background/70 p-3 text-sm tabular-nums text-primary outline-none transition focus:border-indigo-500/60 focus:ring-4 focus:ring-indigo-500/10',
  readonlyField: 'flex min-h-11 w-full items-center gap-2 rounded-xl bg-background/70 px-3.5 py-2.5 font-medium text-muted ring-1 ring-inset ring-border/60',
  tabGroup: 'flex gap-1 rounded-2xl bg-background/65 p-1 ring-1 ring-inset ring-border/60',
  tabButton: (active: boolean) => `min-h-11 flex-1 rounded-xl px-3 py-2 text-xs font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/60 ${active ? 'bg-surface text-primary shadow-sm' : 'text-muted hover:text-primary'}`,
  ...
  sectionPanel: 'mt-4 space-y-4 rounded-2xl bg-background/55 p-4 ring-1 ring-inset ring-border/60',
  accentSectionPanel: 'mt-4 space-y-4 rounded-2xl bg-emerald-500/[0.06] p-4 ring-1 ring-inset ring-emerald-500/20',
  accentPanel: 'space-y-4 rounded-2xl bg-emerald-500/[0.06] p-4 ring-1 ring-inset ring-emerald-500/20',
  ...
  helpText: 'mt-2 text-xs leading-relaxed text-muted',
  checkbox: 'h-5 w-5 rounded border-border text-indigo-600 focus:ring-indigo-500',
  primaryButton: 'flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 text-base font-semibold text-white shadow-sm shadow-indigo-500/20 transition-colors hover:bg-indigo-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/70 focus-visible:ring-offset-2 focus-visible:ring-offset-surface disabled:cursor-not-allowed disabled:opacity-50',
} as const;
