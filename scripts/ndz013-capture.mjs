...
const outDir = path.join(root, 'docs', 'ndz-013-screenshots');
await mkdir(outDir, { recursive: true });

const userDataDir = '/tmp/ndz013-chrome-profile';
...
const db = {
  data: [
    item('t1', 'TODO', 'Verify NDZ-013 desktop status visibility', 'pending', 0, { date: today, priority: 'high', tags: ['ndz','desktop'] }),
    item('t2', 'TODO', 'Keep sync + review status visible in desktop rail', 'pending', 0, { date: today, priority: 'normal', tags: ['qa'] }),
    ...
    item('n1', 'NOTE', 'Status zone should stay explicit in the desktop rail.', 'done', -1, { tags: ['layout'] }),
    item('n2', 'NOTE', 'Review queue count and manual actions should stay one click away.', 'done', -1, { tags: ['library'] }),
...
async function collectMetrics(page, label, width, height) {
  const res = await page.send('Runtime.evaluate', {
    expression: `(() => {
      ...
      const composer = document.querySelector('[data-shell-composer-variant]');
      const composerRect = composer?.getBoundingClientRect();
      const chat = document.querySelector('[class*="bottom-full"]');
      ...
      return {
        ...
        composerVariant: composer?.getAttribute('data-shell-composer-variant') || null,
        composerLeft: composerRect ? Number(composerRect.left.toFixed(1)) : null,
        composerWidth: composerRect ? Number(composerRect.width.toFixed(1)) : null,
        chatLeft: chatRect ? Number(chatRect.left.toFixed(1)) : null,
        chatWidth: chatRect ? Number(chatRect.width.toFixed(1)) : null,
      };
    })()`,
    returnByValue: true,
  });
